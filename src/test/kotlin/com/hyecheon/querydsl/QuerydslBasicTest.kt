...
class QuerydslBasicTest {
	...
	@Test
	internal fun findDtoByQueryProjection() {
		val result = queryFactory
				.select(QMemberDto(member.username, member.age))
				.from(member)
				.fetch()
		result.forEach { println("memberDto = $it") }
	}


	@Test
	internal fun dynamicQueryBooleanBuilder() {
	...
	@Test
	internal fun dynamicQueryWhereParam() {
		val usernameParam = "member1"
		val ageParam = 10
	...
	private fun ageEq(ageCond: Int?): BooleanExpression? {
		return if (ageCond == null) null else member.age.eq(ageCond)
	}
